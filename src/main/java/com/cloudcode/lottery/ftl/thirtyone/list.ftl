...
$(function(){
    if ($.fn.jqGrid){
       grid =  $("#jqGrid01").jqGrid({
           url:"${request.getContextPath()}/thirtyOneHistory/query",
            datatype: "json",
            height: 250,
            rowNum: 20,
...
$( "#openwindow" ).click(function(){
	$( "#divInDialog" ).dialog({
		...
		open: function(event, ui) {
  			 $('#divInDialog').load('${request.getContextPath()}/thirtyOneHistory/toCreate', function() {
...
$( "#edit" ).click(function(){
	...
	$( "#divInDialog" ).dialog({
	 	 ...
	 	 width:800,
		open: function(event, ui) {
  			$('#divInDialog').load('${request.getContextPath()}/thirtyOneHistory/'+id+'/toUpdate', function() {
 		 	 });
	  },	   
    close: function (event, ui) {  
       grid.trigger("reloadGrid");
...
if(id.toString() != null && id.toString() != ""){
		    $.ajax({
			        url: '${request.getContextPath()}/thirtyOneHistory/'+id+'/delete',
			        type: 'post',
			        dataType: 'json',
	...
	$( "#divInDialog" ).dialog({
	 	 ...
	 	 width:800,
		open: function(event, ui) {
  			$('#divInDialog').load('${request.getContextPath()}/thirtyOneHistory/'+id+'/toView', function() {
 		 	 });
	  },	   
    close: function (event, ui) {  
       grid.trigger("reloadGrid");
	...
	$("#layout button,.button,#sampleButton").button();
	 $( "#export" ).click(function(){
			ajaxframework.postForm('${request.getContextPath()}/thirtyOneHistory/export',null,null)
	});
